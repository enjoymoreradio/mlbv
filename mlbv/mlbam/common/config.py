...
class Config:
        ...
        for config_dir_base in Config.config_dir_roots:
            for config_dir_name in (script_name, "." + script_name):
                ...
                if (
                    ...
                    and os.path.exists(os.path.join(test_dir, "config"))
                ):
                    config_dir = test_dir
                    break
    ...
    def generate_config(username=None, password=None, servicename="MLB.tv"):
        ...
        if os.path.exists(config_file):
            print(
                "Aborting: The config file already exists at '{}'".format(config_file)
            )
            return False
        ...
        template_config_path = os.path.abspath(
            os.path.join(current_dir, "../../..", "config.template")
        )
        if not os.path.exists(template_config_path):
            print(
                "Could not find template config file [expected at: {}]".format(
